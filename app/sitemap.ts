...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const paths = new Set<string>(
    (Object.keys(SEO_PAGES) as StaticSeoPath[]).filter(
      (path) => !seoPage(path).noindex,
    ),
  );
  for (const product of products)
    paths.add(`/product/${encodeURIComponent(product.id)}`);
  for (const category of categories)
    if (category.count) paths.add(category.href);
  const populatedDistricts = new Set(
    catalog.map((product) => product.district).filter(Boolean),
  );
  for (const district of [...DISTRICTS_SEED, ...districts]) {
    if (populatedDistricts.has(district.slug))
      paths.add(`/district/${encodeURIComponent(district.slug)}`);
  }
  paths.add("/restaurants");
  const restaurants =
    await sql`SELECT id FROM restaurants WHERE approval='approved'`;
  for (const restaurant of restaurants)
    paths.add(`/restaurants/${encodeURIComponent(restaurant.id)}`);
