import OrderTracking from "@/components/platform/OrderTracking";
...
export default async function OrderPage({
  ...
  const [{ id }, { t }, customer] = await Promise.all([
    params,
    searchParams,
    getCurrentCustomer(),
  ]);
        ...
        <div className="mx-auto max-w-2xl px-4 py-12 sm:px-6">
          <div className="rounded-xl bg-white p-8 shadow-sm ring-1 ring-black/5">
            <p className="text-sm text-gray-600">
              We couldn&apos;t match that order number and phone number. Check
              both against your confirmation and try again.
            </p>
            <div className="mt-6">
  ...
  const products = await Promise.all(
    order.items.map((it) => getProduct(it.id)),
  );
  const buyAgain = order.items.flatMap((it, i) => {
    ...
    return product &&
      ...
      (it.addonIds ?? []).every((id) =>
        product.addons?.some((a) => a.id === id),
      )
      ? [
          {
            product: {
          ...
          <section className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-black/5 lg:col-span-2">
              ...
              {order.items.map((it, i) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <Link
                      href={`/product/${encodeURIComponent(it.id)}`}
                      className="text-sm font-medium text-gray-900 hover:text-brand"
                    >
                      {it.name}
                    </Link>
                    <p className="text-xs text-gray-500">
                      ₹{it.price.toFixed(0)} × {it.qty}
                    </p>
                    {it.instructions ? (
                      <p className="text-xs text-gray-500">{it.instructions}</p>
                    ) : null}
                  </div>
            ...
            <dl className="mt-4 space-y-2 border-t border-black/10 pt-4 text-sm">
              <div className="flex justify-between">
                ...
                <dd>₹{order.subtotal.toFixed(0)}</dd>
              </div>
              <div className="flex justify-between">
