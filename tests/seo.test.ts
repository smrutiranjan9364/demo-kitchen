...
test("indexing requires an explicit production origin and opt-in", () => {
  assert.deepEqual(getSeoConfig({ NODE_ENV: "production" }), {
    siteUrl: undefined,
    indexable: false,
  });
  assert.equal(
    getSeoConfig({ ...production, SEO_INDEXABLE: undefined }).indexable,
    false,
  );
  assert.equal(
    getSeoConfig({ ...production, SEO_INDEXABLE: "false" }).indexable,
    false,
  );
  assert.equal(
    getSeoConfig({ ...production, SEO_INDEXABLE: "TRUE" }).indexable,
    false,
  );
  assert.equal(getSeoConfig(production).indexable, true);
  assert.equal(
    getSeoConfig({ ...production, SITE_URL: `  ${ORIGIN}/  ` }).siteUrl,
    ORIGIN,
  );
  assert.throws(
    () => getSeoConfig({ NODE_ENV: "production", SEO_INDEXABLE: "true" }),
    /SITE_URL/,
  );
});
...
test("development, test and provider preview deployments cannot opt into indexing", () => {
  for (const overrides of [
    ...
    assert.equal(
      getSeoConfig({ ...production, ...overrides } as NodeJS.ProcessEnv)
        .indexable,
      false,
...
test("invalid, private, preview and non-origin SITE_URL values are rejected", () => {
  for (const siteUrl of [
    ...
    "https://singlelabel",
    "https://user:password@kitchen.example.com",
    `${ORIGIN}:8443`,
    `${ORIGIN}/shop`,
    `${ORIGIN}?tracking=1`,
    `${ORIGIN}#fragment`,
    ...
    assert.throws(
      () => getSeoConfig({ ...production, SITE_URL: siteUrl }),
      siteUrl,
    );
  }
});
...
test("absolute URLs keep page paths on the canonical origin", async () => {
  await withEnvironment(production, () => {
    ...
    assert.equal(
      absoluteUrl("/product/chhena-poda"),
      `${ORIGIN}/product/chhena-poda`,
    );
    for (const unsafe of [
      "shop",
...
test("metadata includes matching canonical, Open Graph and Twitter fields", async () => {
  await withEnvironment(production, () => {
    ...
    assert.equal(
      (metadata.metadataBase as URL | undefined)?.href,
      `${ORIGIN}/`,
    );
    ...
    assert.deepEqual(metadata.openGraph, {
      title: `Odia Food | ${SITE_NAME}`,
      description: "Browse traditional food.",
      url: `${ORIGIN}/shop`,
      siteName: SITE_NAME,
      type: "website",
      locale: "en_IN",
...
test("static routes have distinct titles and descriptions; utility pages are noindex", async () => {
  await withEnvironment(production, () => {
    ...
    assert.equal(
      new Set(metadata.map((entry) => JSON.stringify(entry.title))).size,
      paths.length,
    );
    assert.equal(
      new Set(metadata.map((entry) => entry.description)).size,
      paths.length,
    );
    for (const path of [
      "/cart",
...
test("preview metadata and sitemap remain nonindexable with the production origin configured", async () => {
  await withEnvironment({ ...production, VERCEL_ENV: "preview" }, async () => {
    assert.equal(
      (routeMetadata("/").robots as { index: boolean }).index,
      false,
    );
...
test("social images support HTTPS or local assets and reject unsafe URLs", async () => {
  await withEnvironment(production, () => {
    ...
    for (const unsafe of [
      "http://images.example.com/food.jpg",
      "javascript:alert(1)",
    ...
    const metadata = createMetadata({
      ...seoPage("/shop"),
      image: "https://images.example.com/food.jpg",
      imageAlt: "A food collection",
    });
    assert.deepEqual(metadata.openGraph?.images, [
      { url: "https://images.example.com/food.jpg", alt: "A food collection" },
    ]);
  });
});
...
test("search and filter variants are flagged without noindexing campaign parameters", () => {
  ...
  assert.equal(
    isSearchVariant({ utm_source: "newsletter", gclid: "campaign-id" }),
    false,
  );
});
...
test("empty collections cannot enter the index and dynamic paths encode identifiers", () => {
  ...
  assert.equal(
    productSeo({ ...BEST_SELLERS[0], id: "a/b?c#d" }).path,
    "/product/a%2Fb%3Fc%23d",
  );
  assert.equal(
    districtSeo({ ...district, slug: "a/b" }, 1).path,
    "/district/a%2Fb",
  );
});
...
test("site schema identifies the visible brand without invented business attributes", async () => {
  await withEnvironment(production, () => {
    ...
    for (const field of [
      ...
      assert.equal(organization[field], undefined);
...
test("product and breadcrumb schema use actual values without demo reviews or inferred stock", async () => {
  await withEnvironment(production, () => {
    ...
    for (const field of [
      ...
      assert.equal(offer[field], undefined);
    ...
    assert.deepEqual(
      ...
      [`${ORIGIN}/`, `${ORIGIN}/category/snacks`, `${ORIGIN}${page.path}`],
    );
    for (const price of [Number.NaN, Number.POSITIVE_INFINITY, -1]) {
      const invalid = jsonLdData(
        PageJsonLd({ page, product: { ...product, price } }),
      );
      assert.equal(
        invalid["@graph"].find((entry) => entry["@type"] === "Product")!.offers,
        undefined,
      );
    }
  });
...
test("catalog identity and sitemap share canonical products without resurrecting deleted seeds", async () => {
  ...
  dbGlobal.sql = async (parts: TemplateStringsArray, ...values: unknown[]) => {
      ...
      return [
        {
          ...
          image: "/logo.png",
          description: null,
          count: products.filter((product) => product.category === "snacks")
            .length,
        },
        {
          slug: "empty",
    ...
    if (query.includes("FROM restaurants")) return [{ id: "odia-kitchen" }];
    if (query.includes("FROM districts"))
      return [
        {
          slug: "puri",
          name: "Puri",
    ...
    if (query.includes("FROM products WHERE id"))
      return products.filter((product) => product.id === values[0]).map(row);
    if (query.includes("WHERE category") && query.includes("id <>"))
      return products
        .filter(
          (product) =>
            product.category === values[0] && product.id !== values[1],
        )
  ...
  try {
    ...
    for (const featured of BEST_SELLERS) {
      const seed = ALL_PRODUCTS.find(
        (product) =>
          product.name === featured.name && product.image === featured.image,
      )!;
      assert.equal(
        (await getProductPageData(featured.id))?.canonicalId,
        seed.id,
      );
    ...
    products = products.map((product) =>
      product.id === "sweets-1"
        ? { ...product, price: 999, district: "puri" }
        : product,
    );
    assert.equal(
      (await getFeaturedProducts()).bestSellers.find(
        (product) => product.id === "sweets-1",
      )?.price,
      999,
    );
    await withEnvironment(production, async () => {
      ...
      assert.ok(urls.includes(`${ORIGIN}/restaurants/odia-kitchen`));
      ...
      for (const path of [
        ...
        assert.ok(!urls.includes(`${ORIGIN}${path}`), path);
    ...
    assert.equal(
      (await getProductPageData("chhena-poda"))?.canonicalId,
      "chhena-poda",
    );
    const seed = ALL_PRODUCTS.find((product) => product.id === "sweets-1")!;
    for (const custom of [
      { ...seed, id: "unrelated-custom-id" },
      { ...seed, name: "Different product" },
    ]) {
      products = [custom];
      assert.equal(
        (await getProductPageData("chhena-poda"))?.canonicalId,
